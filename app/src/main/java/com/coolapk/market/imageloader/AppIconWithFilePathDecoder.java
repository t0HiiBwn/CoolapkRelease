    ...
    public Resource<Bitmap> decode(Uri uri, int i, int i2, Options options) {
        ...
        try {
            ...
            if (str != null) {
                if (str.length() != 0) {
                    z = false;
                    if (!z && (packageArchiveInfo = this.context.getPackageManager().getPackageArchiveInfo(path, 0)) != null) {
