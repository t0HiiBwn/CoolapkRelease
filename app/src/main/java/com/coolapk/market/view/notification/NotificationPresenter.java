...
public class NotificationPresenter extends NotificationContract.Presenter {
    ...
    public NotificationPresenter(NotificationContract.View view2) {
        super(view2);
        this.view = view2;
    }
    ...
    protected Observable<Result<List<Entity>>> onCreateRequest(final boolean z, int i) {
        return DataManager.getInstance().getNotificationList(i, this.view.findFirstItem(), this.view.findLastItem()).map(RxUtils.checkResult()).doOnNext(new Action1<Result<List<Entity>>>() {
            /* class com.coolapk.market.view.notification.NotificationPresenter.AnonymousClass1 */
