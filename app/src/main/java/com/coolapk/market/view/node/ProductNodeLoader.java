...
public final class ProductNodeLoader implements Loader {
    ...
    public boolean checkIdentity() {
        return this.intent.hasExtra("PRODUCT_ID");
    }

    @Override // com.coolapk.market.view.node.Loader
    public Observable<Result<Entity>> loadData() {
        Observable<R> map = DataManager.getInstance().getProductDetail(this.intent.getStringExtra("PRODUCT_ID")).map(ProductNodeLoader$loadData$1.INSTANCE);
        Intrinsics.checkNotNullExpressionValue(map, "DataManager.getInstance(….cast<Result<Entity>>() }");
        return map;
    }
}
