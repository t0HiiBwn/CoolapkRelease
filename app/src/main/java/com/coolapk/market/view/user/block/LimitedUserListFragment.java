...
public class LimitedUserListFragment extends BlockedUserListFragment {
    ...
    public Observable<Result<String>> onCancelClick(User user) {
        return DataManager.getInstance().removeFromLimitList(user.getUid());
    }

    @Override // com.coolapk.market.view.base.asynclist.NewAsyncListContract.View
    public Observable<Result<List<User>>> onCreateRequest(boolean z, int i) {
        return DataManager.getInstance().getMyLimitListUser(i, findFirstItem(), findLastItem()).compose(RxUtils.applyIOSchedulers());
    }
}
