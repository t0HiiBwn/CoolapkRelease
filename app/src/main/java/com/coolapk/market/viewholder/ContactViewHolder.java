...
public class ContactViewHolder extends GenericBindHolder<ItemContactBinding, Contacts> {
    ...
    public void bindToContent(Contacts contacts) {
        ...
        if (DateUtils.getSomeDateTime() < loginTime) {
            ((ItemContactBinding) getBinding()).setLoginTime(getContext().getString(2131887100, DateUtils.getTimeDescription(getContext(), Long.valueOf(loginTime))));
        }
        ItemContactBinding itemContactBinding = (ItemContactBinding) getBinding();
        if (this.listType > 0) {
            str = StringUtils.emptyIfNull(getContext().getString(2131887092, followerUserInfo.getFollow()));
        } else {
            str = StringUtils.emptyIfNull(getContext().getString(2131887092, userInfo.getFollow()));
        }
        itemContactBinding.setFollow(str);
        ItemContactBinding itemContactBinding2 = (ItemContactBinding) getBinding();
        if (this.listType > 0) {
            str2 = StringUtils.emptyIfNull(getContext().getString(2131887088, followerUserInfo.getFans()));
        } else {
            str2 = StringUtils.emptyIfNull(getContext().getString(2131887088, userInfo.getFans()));
        }
    ...
    public void onClick(View view) {
        ...
        super.onClick(view);
        int id = view.getId();
        if ((id == 2131362200 || id == 2131362672) && this.listType != 0) {
            ActionManager.startUserSpaceActivity(this.itemView.findViewById(2131362672), this.listType > 0 ? model.getFollowUid() : model.getUid(), this.listType > 0 ? model.getFollowerUserAvatar() : model.getUserAvatar());
        }
    }
}
