...
class CameraScanAnalysis implements Camera.PreviewCallback {
    ...
    private Runnable mAnalysisTask = new Runnable() {
        /* class com.coolapk.market.widget.camera.CameraScanAnalysis.AnonymousClass2 */
    ...
    CameraScanAnalysis(Context context2) {
        ...
        this.mHandler = new Handler(Looper.getMainLooper()) {
            /* class com.coolapk.market.widget.camera.CameraScanAnalysis.AnonymousClass1 */
    ...
    public void onPreviewFrame(byte[] bArr, Camera camera2) {
        if (this.allowAnalysis) {
            ...
            Image image = new Image(previewSize.width, this.size.height, "Y800");
            this.barcode = image;
            image.setData(bArr);
            if (Symbol.is_only_scan_center) {
                ...
                this.cropX = (int) (((float) Symbol.cropX) * (((float) this.size.height) / ((float) Symbol.screenWidth)));
                int i = (int) (((float) Symbol.cropY) * (((float) this.size.width) / ((float) Symbol.screenHeight)));
                this.cropY = i;
                this.barcode.setCrop(i, this.cropX, this.cropHeight, this.cropWidth);
            } else {
                Symbol.cropX = 0;
                Symbol.cropY = 0;
